...
-- Procedures
--
CREATE DEFINER=`dmr1674`@`localhost` PROCEDURE `SelectAllClients` ()  SELECT * FROM Client$$

--
-- Functions
--
CREATE DEFINER=`dmr1674`@`localhost` FUNCTION `getProfit` (`revenue` FLOAT, `cost` FLOAT) RETURNS FLOAT(9,2) BEGIN
  DECLARE profit FLOAT(9,2);
...
(7438, 'Walmart', '3334551212', 'Walmart@walmart.com'),
...
(7, 'WALMART', '2021-03-04', '2021-06-22', 20000, 30000, 'Demolishing UNCW to build Walmart', 7438, 3);

--
-- Triggers `Project`
--
DELIMITER $$
CREATE TRIGGER `formatProjectName` BEFORE INSERT ON `Project` FOR EACH ROW SET NEW.name = UPPER(NEW.name)
$$
DELIMITER ;
